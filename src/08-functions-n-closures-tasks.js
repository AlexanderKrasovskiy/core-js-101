...
function getPolynom(...args) {
  return function y(x) {
    ...
    const last = newArgs[0] ? newArgs[0] : 0;
    const mid = newArgs[1] ? newArgs[1] : 0;
    const first = newArgs[2] ? newArgs[2] : 0;

    return first * (x ** 2) + mid * x + last;
  };
}
...
function logger(func, logFunc) {
  ...
  return function a(...args) {
    const newArgs = JSON.stringify(args).slice(1, -1);
    const start = `${name}(${newArgs}) starts`;
    const end = `${name}(${newArgs}) ends`;
